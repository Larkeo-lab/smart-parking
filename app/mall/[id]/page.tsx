...
// Types
import { Reservation, Spot, Mall } from "@/types/parking";
...
export default function MallDetailPage() {
  ...
  // Load mall data
  const mall = useMemo(
    () => initialMalls.find((m: Mall) => m.id === mallId),
    [mallId],
  );
  ...
  const handleSelectSpot = (mId: string, sId: number) => {
    setSelectedSpot({ mallId: mId, spotId: sId });
    onOpen();
  };

  const handleGateControl = async (open: boolean) => {
    ...
    if (activeReservation) {
      ...
      const updatedReservations = currentReservations.map((r: Reservation) =>
        r.id === activeReservation.id ? { ...r, gateOpen: open } : r,
      );
  ...
  const cancelReservation = async () => {
    ...
    // Remove from Supabase (or mark as cancelled)
    ...
    const updatedReservations = currentReservations.filter(
      (r: Reservation) => r.id !== activeReservation.id,
    );
    localStorage.setItem("reservations", JSON.stringify(updatedReservations));
  ...
  const confirmPayment = async () => {
    ...
    setIsGateOpen(false);
    setHasPaid(true); // Only show card after payment
    onClose();
  };
